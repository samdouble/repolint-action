...
// src/rules/json-has-keys.ts
var import_zod7 = require("zod");

// src/utils/has-keys.ts
var getNestedValue = (obj, keyPath) => {
...
var hasKey = (obj, keyPath) => {
  const value = getNestedValue(obj, keyPath);
  return value !== void 0;
};
var checkHasKeys = async (context, options, parse, formatName) => {
  const errors = [];
  const { path: path2, keys } = options;
  const paths = Array.isArray(path2) ? path2 : [path2];
  for (const pathPattern of paths) {
    const isPattern = isGlobPattern(pathPattern);
    let filesToCheck;
...
// src/rules/json-has-keys.ts
var JsonHasKeysOptionsSchema = import_zod7.z.object({
  path: import_zod7.z.union([import_zod7.z.string(), import_zod7.z.array(import_zod7.z.string()).min(1)]),
  keys: import_zod7.z.array(import_zod7.z.string()).min(1)
});
var JsonHasKeysSchema = import_zod7.z.object({
  name: import_zod7.z.literal("json-has-keys"),
  level: AlertLevelSchema,
  options: JsonHasKeysOptionsSchema
});
var jsonHasKeys = async (context, ruleOptions) => {
  let sanitizedRuleOptions;
  try {
...
var LicenseExistsOptionsSchema = import_zod8.z.object({
  caseSensitive: import_zod8.z.boolean().default(false),
  path: import_zod8.z.union([import_zod8.z.string(), import_zod8.z.array(import_zod8.z.string()).min(1)]).default("LICENSE.md")
});
var LicenseExistsSchema = import_zod8.z.object({
  name: import_zod8.z.literal("license/exists"),
  level: AlertLevelSchema,
...
var PyprojectDependenciesAlphabeticalOrderOptionsSchema = import_zod9.z.object({
  path: import_zod9.z.string().default("pyproject.toml"),
  sections: import_zod9.z.array(import_zod9.z.string()).default(["project.dependencies", "project.optional-dependencies", "tool.poetry.dependencies"])
});
var PyprojectDependenciesAlphabeticalOrderSchema = import_zod9.z.object({
  name: import_zod9.z.literal("python/pyproject-dependencies-alphabetical-order"),
  level: AlertLevelSchema,
...
var pyprojectDependenciesAlphabeticalOrder = async (context, ruleOptions = {}) => {
  ...
  for (const sectionPath of sections) {
    const sectionValue = getNestedValue2(pyproject, sectionPath);
...
var ReadmeExistsOptionsSchema = import_zod10.z.object({
  caseSensitive: import_zod10.z.boolean().optional().default(false),
  path: import_zod10.z.union([import_zod10.z.string(), import_zod10.z.array(import_zod10.z.string()).min(1)]).optional().default("README.md")
});
var ReadmeExistsSchema = import_zod10.z.object({
  name: import_zod10.z.literal("readme/exists"),
  level: AlertLevelSchema,
...
// src/rules/requirements-txt-dependencies-alphabetical-order.ts
var RequirementsTxtDependenciesAlphabeticalOrderOptionsSchema = import_zod11.z.object({
  path: import_zod11.z.string().default("requirements.txt")
});
var RequirementsTxtDependenciesAlphabeticalOrderSchema = import_zod11.z.object({
  name: import_zod11.z.literal("python/requirements-txt-dependencies-alphabetical-order"),
  level: AlertLevelSchema,
...
// src/rules/yaml-has-keys.ts
var import_yaml2 = require("yaml");
var import_zod12 = require("zod");
var YamlHasKeysOptionsSchema = import_zod12.z.object({
  path: import_zod12.z.union([import_zod12.z.string(), import_zod12.z.array(import_zod12.z.string()).min(1)]),
  keys: import_zod12.z.array(import_zod12.z.string()).min(1)
});
var YamlHasKeysSchema = import_zod12.z.object({
  name: import_zod12.z.literal("yaml-has-keys"),
  level: AlertLevelSchema,
  options: YamlHasKeysOptionsSchema
});
var yamlHasKeys = async (context, ruleOptions) => {
  let sanitizedRuleOptions;
  try {
  ...
  return checkHasKeys(context, sanitizedRuleOptions, import_yaml2.parse, "YAML");
};
...
var ruleConfigSchema = import_zod13.default.union([
  FileContainsSchema,
...
var rulesConfigSchema = import_zod13.default.array(ruleConfigSchema);

// src/utils/config.ts
var regexPatternSchema = import_zod14.z.string().refine(
  (pattern) => {
    try {
      new RegExp(pattern);
      return true;
...
var filtersSchema = import_zod14.z.object({
  archived: import_zod14.z.boolean().optional(),
  organizations: import_zod14.z.array(import_zod14.z.string()).optional(),
  visibility: import_zod14.z.enum(["public", "private", "all"]).optional(),
  include: import_zod14.z.array(regexPatternSchema).optional(),
  exclude: import_zod14.z.array(regexPatternSchema).optional()
});
var configSchema = import_zod14.z.object({
...
var packageJson = JSON.parse(
  (0, import_node_fs.readFileSync)((0, import_node_path3.join)(__dirname, "../package.json"), "utf-8")
);
var program = new import_commander.Command();
program.name("repofmt").description("A CLI tool that checks consistency across repositories").version(packageJson.version);
program.command("run").description("Run repofmt checks against your repositories").option("--config <path>", "path to config file").option("--token <token>", "GitHub token (can also use GITHUB_TOKEN env var)").action(async (options) => {
